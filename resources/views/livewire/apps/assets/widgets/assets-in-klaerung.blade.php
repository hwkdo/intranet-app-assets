...
new class extends Component
{
    ...
    {
        $value = auth()->user()?->settings->app->assets->dashboard['widgetItemCounts']['assets-in-klaerung']
            ?? auth()->user()?->settings->dashboard->personalGrid?->widgetItemCounts['assets-in-klaerung']
            ?? 5;
...
>
    ...
    @empty
        <flux:text class="text-zinc-500 dark:text-white">Keine Assets in Klärung vorhanden.</flux:text>
    @endforelse

    @if($this->hasMore)
        <div class="pt-1">
            <flux:button variant="ghost" size="sm" :href="route('apps.assets.admin.clarifications')" wire:navigate>
                Weitere anzeigen
            </flux:button>
        </div>
    @endif
