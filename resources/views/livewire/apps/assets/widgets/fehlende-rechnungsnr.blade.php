...
new class extends Component
{
    ...
    {
        $value = auth()->user()?->settings->app->assets->dashboard['widgetItemCounts']['fehlende-rechnungsnr']
            ?? auth()->user()?->settings->dashboard->personalGrid?->widgetItemCounts['fehlende-rechnungsnr']
            ?? 5;
...
<x-intranet-app-base::dashboard.widget-card
    :title="'Fehlende Rechnungsnr ('.$this->totalCount().')'"
    :description="'Assets ohne Rechnungsnummer (max. '.$this->itemLimit().')'"
>
    @forelse($this->assets as $asset)
        <a
            href="{{ route('apps.assets.show', [$asset, 'from' => 'fehlende-rechnung']) }}"
            wire:navigate
    ...
    @empty
        <flux:text class="text-zinc-500 dark:text-white">Keine Assets mit fehlender Rechnungsnummer vorhanden.</flux:text>
    @endforelse

    @if($this->hasMore)
        <div class="pt-1">
            <flux:button variant="ghost" size="sm" :href="route('apps.assets.fehlende-rechnung')" wire:navigate>
                Weitere anzeigen
            </flux:button>
        </div>
    @endif
