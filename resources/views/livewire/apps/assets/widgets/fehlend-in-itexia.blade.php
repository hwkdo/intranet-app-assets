...
>
    @forelse($this->assets as $asset)
        <a
            href="{{ route('apps.assets.show', [$asset, 'from' => 'itexiageraete']) }}"
            wire:navigate
    ...
    @empty
        <flux:text class="text-zinc-500 dark:text-white">Keine fehlenden Itexia-Zuordnungen vorhanden.</flux:text>
    @endforelse
    ...
    @if($this->hasMore)
        <div class="pt-1">
            <flux:button
                variant="ghost"
                size="sm"
                :href="route('apps.assets.itexiageraete', ['itexiaFilter' => 'not-found'])"
                wire:navigate
            >
                Weitere anzeigen
            </flux:button>
