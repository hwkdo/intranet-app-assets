...
<div>
    <x-intranet-app-assets::assets-layout
        heading="Abgelehnte Übergaben"
        subheading="Vom Empfänger abgelehnte Übergaben — bitte Besitz und Standort klären"
    >
        <div
            class="space-y-4"
            x-data="{
                                ...
                                <div x-show="selectedIds.length > 0" x-cloak class="space-y-4">
                                    <div class="grid gap-3 md:grid-cols-2">
                                        <flux:radio.group wire:model.live="resolution" label="Aktion">
                                            <flux:radio value="{{ \Hwkdo\IntranetAppAssets\Services\HandoverRejectionAdminResolutionService::ResolutionNewOwner }}" label="Neuen Besitzer zuweisen" />
                                            <flux:radio value="{{ \Hwkdo\IntranetAppAssets\Services\HandoverRejectionAdminResolutionService::ResolutionSetLocation }}" label="Besitzer entfernen und Standort setzen" />
                                            <flux:radio value="{{ \Hwkdo\IntranetAppAssets\Services\HandoverRejectionAdminResolutionService::ResolutionMarkMissing }}" label="Als vermisst markieren" />
                                        </flux:radio.group>
                                        <flux:textarea wire:model="bulkReason" label="Grund / Notiz (für alle ausgewählten)" rows="3" />
                                    </div>

                                    @if($resolution === \Hwkdo\IntranetAppAssets\Services\HandoverRejectionAdminResolutionService::ResolutionNewOwner)
                                    ...
                                    @if($resolution === \Hwkdo\IntranetAppAssets\Services\HandoverRejectionAdminResolutionService::ResolutionSetLocation)
            ...
            <flux:table>
                ...
                <flux:table.rows>
                    @forelse($handovers as $handover)
                        @php
                            $reasonNote = $handover->notes->first();
                            $reasonPreview = $reasonNote ? \Illuminate\Support\Str::limit(strip_tags($reasonNote->note), 120) : '—';
                        @endphp
                        <flux:table.row wire:key="rejected-ho-{{ $handover->id }}">
                            <flux:table.cell>
                                <label class="inline-flex cursor-pointer items-center">
                                    <input
                                        type="checkbox"
                            ...
                            <flux:table.cell>{{ $handover->rejected_at?->format('d.m.Y H:i') ?? '—' }}</flux:table.cell>
                            <flux:table.cell>
                                @if($handover->asset)
